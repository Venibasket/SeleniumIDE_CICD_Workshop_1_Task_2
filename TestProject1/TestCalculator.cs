...
namespace TestProject1
{
    ...
    public class TestCalculator
    {
        ...
        public void SetUp()
        {
            ...
            // Safely get the project directory, checking for nulls to avoid CS8602
            DirectoryInfo? baseDir = Directory.GetParent(AppContext.BaseDirectory);
            if (baseDir == null || baseDir.Parent == null || baseDir.Parent.Parent == null)
            {
                throw new InvalidOperationException("Unable to determine project directory from AppContext.BaseDirectory.");
            }
            string projectDir = baseDir.Parent.Parent.Parent.FullName;

            // 🔹 Build path to the HTML file in Calculator folder
            string calcPath = Path.Combine(projectDir, "Calculator", "number-calculator.html");
            string calcUrl = new Uri(calcPath).AbsoluteUri;

            // Optional debug, just to see it once:
            Console.WriteLine($"Calc path: {calcPath}");
            Console.WriteLine($"Exists: {File.Exists(calcPath)}");
